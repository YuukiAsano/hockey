...
-(void)update:(ccTime)dt {
    ...
    if(num == 1){
      isHit = [[GameScene sharedInstance].player2 hitIfCollided:self.position player_num:1 item_num:1];
    }else if(num == 2){
      isHit = [[GameScene sharedInstance].player1 hitIfCollided:self.position player_num:2 item_num:1];
    }
    //当たり判定のチェック
    if(isHit){
