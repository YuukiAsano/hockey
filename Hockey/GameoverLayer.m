...
-(id)init {
    ...
    if(self){
        ...
        CCLabelTTF *label = [CCLabelTTF labelWithString:@"GAME OVER" fontName:@"Helvetica" fontSize:64];
        CGSize winSize = [[CCDirector sharedDirector] winSize];
        label.position = ccp(winSize.width/2, winSize.height/2);
        [self addChild:label];
        
        //画面中央の下部にリプレイを促すラベルを表示します。
        CCLabelTTF *replayLabel = [CCLabelTTF labelWithString:@"Tap to replay" fontName:@"Helvetica" fontSize:32];
